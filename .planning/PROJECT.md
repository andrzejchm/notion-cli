...
- [ ] Internal integration token auth with `ntn init` setup
...
- Command name: `ntn` (short, avoids conflict with potential official `notion` CLI)
...
- **Auth**: Internal integration token only (`ntn_...` from Notion integration settings)
...
| `ntn` as command name | Short, memorable, avoids conflict with potential official CLI | — Pending |
